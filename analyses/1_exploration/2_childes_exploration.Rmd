...
get_num_core_words <- function(this_target_child_id, this_age_bin, df, core_words) {
  ...
  n_core_types_non_cumulative <- non_cumulative_df  %>%
    filter(gloss %in% core_words) %>%
    summarize(n_core_types = n())  %>%
    select(n_core_types) %>%
    pull()
  
  data.frame(target_child_id = this_target_child_id,
              numeric_age_bin = this_age_bin, 
              n_types = nrow(non_cumulative_df),
              n_types_cumulative = nrow(cumulative_df),
              n_tokens = sum(non_cumulative_df$count),
              n_tokens_cumulative = sum(cumulative_df$count),
              n_core_types = ifelse(length(n_core_types_non_cumulative) > 0 , n_core_types_non_cumulative, NA),
...
ggplot(all_counts, aes(x = n_core_types_cumulative, 
                       fill = as.factor(numeric_age_bin))) +
  geom_histogram() +
  facet_grid(~numeric_age_bin) +
  theme(legend.position = "none")


all_counts %>%
  group_by(numeric_age_bin) %>%
  multi_boot_standard(col = "n_core_types", na.rm = T) %>%
  ggplot(aes(x = numeric_age_bin, y = mean)) +
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper, color = as.factor(numeric_age_bin))) +
  ylab("N core types non-cumulative") +
  theme(legend.position = "none")

all_counts %>%
  group_by(numeric_age_bin) %>%
  multi_boot_standard(col = "n_core_types_cumulative", na.rm = T) %>%
  mutate(fct_age_bin = as.factor(numeric_age_bin),
        age_bin = fct_recode(fct_age_bin, 
                             "18m" = "1", "24m" = "2", "32m" = "3", "48m" = "4")) %>%
  ggplot(aes(x = age_bin, y = mean)) +
  ggtitle("Cumulative # Kernal words") +
  geom_pointrange(aes(ymin = ci_lower, ymax = ci_upper, color = as.factor(numeric_age_bin))) +
  ylab("N core types cumulative") +
    theme(legend.position = "none", text = element_text(size = 18))
...
all_counts_with_resids <-  all_counts %>%
  add_residuals(lm(n_types~n_tokens, d = all_counts),
                var = "non_cumulative_vocab_resid") %>%
  add_residuals(lm(n_types_cumulative ~ n_tokens_cumulative, d = all_counts), 
                var = "cumulative_vocab_resid") %>%
  add_residuals(lm(n_core_types~n_tokens, d = all_counts), 
                var = "non_cumulative_core_resid") %>%
  ...
  mutate(prop_core_cumulative = cumulative_core_resid/cumulative_vocab_resid,
         prop_core_non_cumulative = non_cumulative_core_resid/non_cumulative_vocab_resid,
         previous_prop_core_cumulative = lag(prop_core_cumulative), 
         previous_prop_core_non_cumulative = lag(prop_core_non_cumulative),
         next_non_cumulative_vocab_resid = lead(non_cumulative_vocab_resid),
         previous_cumulative_core_resid = lag(cumulative_core_resid))

m <- lm(cumulative_vocab_resid ~ previous_cumulative_core_resid  + n_types,
