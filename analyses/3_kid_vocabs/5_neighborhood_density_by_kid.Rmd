---
title: Semantic space by-kid analyses - neighborhood density
date: "`r Sys.Date()`"
output:
  html_document:
    highlight: tango
...
Num corr pairs is correlated at the kid level with mean density at r= .36. But mean density doesn't predict anything; num corr pairs predicts mtld2.
...
mcrae_info <- read_csv("data/mcrae_vocab_by_kid_t1.txt") %>%
  ...
  select(1:14)
#pos_info <- read_csv("data/prop_pos_by_kid_t1.csv")
#conc_info <- read_csv("data/conc_by_kid_t1.csv")
...
```{r}
df_no_corrs <- kid_info %>%
  select(-target_child_id, -target_child_sex, -corpus_name, -collection_name) 
